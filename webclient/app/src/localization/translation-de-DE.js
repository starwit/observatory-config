/* eslint-disable max-len */
const translationDeDE = {
    "annotator.title": "Konfig-UI",
    "point": "Punkt",
    "point.title": "Punkt",
    "point.create.title": "Erstelle Punkt",
    "point.update.title": "Editiere Punkt",
    "point.id": "id",
    "point.xvalue": "xWert*",
    "point.xvalue.hint": "Wert ist erforderlich. Erlaubtes Maximum ist 1. ",
    "point.yvalue": "yWert*",
    "point.yvalue.hint": "Wert ist erforderlich. Erlaubtes Maximum ist 1. ",
    "polygon": "Polygon",
    "polygon.title": "Polygon",
    "polygon.create.title": "Erstelle Polygon",
    "polygon.update.title": "Editiere Polygon",
    "polygon.id": "id",
    "polygon.open": "Öffnen",
    "polygon.open.hint": "",
    "polygon.classification": "Klassifikation",
    "polygon.point": "Punkt",
    "image": "Bild",
    "image.title": "Bild",
    "image.create.title": "Erstelle Bild",
    "image.update.title": "Editiere Bild",
    "image.id": "id",
    "image.src": "src",
    "image.src.hint": "",
    "image.name": "name",
    "image.name.hint": "",
    "image.polygon": "polygon",
    "classification": "Klassifikation",
    "classification.title": "Klassifikation",
    "classification.create.title": "Erstelle Klassifikation",
    "classification.update.title": "Editiere Klassifikation",
    "classification.id": "id",
    "classification.name": "name*",
    "classification.name.hint": "Wert ist erforderlich. ",
    "classification.name.lightbarrier": "Lichtschranke",
    "classification.name.block": "Sperrfläche",
    "classification.name.observe": "Parkbereich",
    "classification.name.flow": "Flussberechnung",
    "observationArea.image.empty": "Bitte fügen Sie ein Kamerabild oder einen Kartenausschnitt als Bild hinzu",
    "observationArea": "Parkfläche",
    "observationAreas.title": "Analysebereiche",
    "observationAreas.loading": "Analysebereiche werden geladen",
    "observationAreas.empty": "Keine Analysebereiche vorhanden, bitte legen Sie eine neue an.",
    "observationAreaCard.noImage": "Kein Bild",
    "observationArea.title": "Parkfläche",
    "observationArea.create.title": "Parkfläche Anlegen",
    "observationArea.update.title": "Parkfläche Bearbeiten",
    "observationArea.copy.title": "Parkfläche Kopieren",
    "observationArea.copy.suffix": "Kopie",
    "observationArea.id": "id",
    "observationArea.name": "Name*",
    "observationArea.name.hint": "Der Name ist ein Pflichtfeld und kann bis zu 150 Zeichen lang sein.",
    "observationArea.saeStreamKey": "SAE Stream Key",
    "observationArea.geoReferenced": "Geokoordinaten",
    "observationArea.geoReferenced.hint": "",
    "observationArea.centerlatitude": "Flächenmittelpunkt Breitengrad",
    "observationArea.centerlatitude.hint": "Der Wert muss zwischen -90 und 90 liegen.",
    "observationArea.centerlongitude": "Flächenmittelpunkt Längengrad",
    "observationArea.centerlongitude.hint": "Der Wert muss zwischen -180 und 180 liegen.",
    "observationArea.topleftlatitude": "Breitengrad Bild oben links",
    "observationArea.topleftlatitude.hint": "Der Wert muss zwischen -90 und 90 liegen.",
    "observationArea.topleftlongitude": "Längengrad Bild oben links",
    "observationArea.topleftlongitude.hint": "Der Wert muss zwischen -180 und 180 liegen.",
    "observationArea.degreeperpixelx": "Längengrade pro Pixel Bildbreite",
    "observationArea.degreeperpixelx.hint": "Der Wert muss zwischen -1 und 1 liegen.",
    "observationArea.degreeperpixely": "Breitengrade pro Pixel Bildhöhe",
    "observationArea.degreeperpixely.hint": "Der Wert muss zwischen -1 und 1 liegen.",
    "observationArea.image": "Bilddatei hier ablegen oder klicken...",
    "observationArea.image.hint": "Optional: Bild ablegen, aus SAE laden oder später hinzufügen.",
    "observationArea.image.preview": "Vorschau",
    "observationArea.image.tooLarge": "Maximale Dateigröße = 4 MB",
    "observationArea.image.invalidType": "Nur JPEG- und PNG-Bilder sind möglich",
    "observationArea.image.unknownError": "Bild konnte nicht geladen werden",
    "observationArea.delete.title": "Parkfläche Löschen?",
    "observationArea.delete.message": "Die Parkfläche und alle enthaltenen Konfigurationen werden unwiderruflich gelöscht.",
    "observationArea.track.start.title": "Datenverarbeitung starten",
    "observationArea.track.stop.title": "Datenverarbeitung stoppen",
    "observationArea.track.start": "Wollen Sie die Verarbeitung starten?",
    "observationArea.track.stop": "Wollen Sie die Konfiguration Speichern und die Verarbeitung der Daten stoppen?",
    "observationArea.showTrajectories": "Live-Ansicht",
    "observationArea.detail.noImage.title": "Noch kein Bild",
    "observationArea.detail.noImage.hint": "Mit \"Bild aktualisieren\" ein Bild aus SAE laden oder die Fläche bearbeiten, um eines hochzuladen.",
    "observationArea.renewImage": "Bild aktualisieren",
    "observationArea.renewImage.geoReferencedDisabled": "Für georeferenzierte Flächen nicht verfügbar.",
    "observationArea.getImageFromSae": "Bild aus SAE laden",
    "app.baseName": "observatory-config",
    "home.title": "Willkommen",
    "home.welcome": "Wilkommen zur Konfigurationsoberfläche für Computer Vision",
    "general.loading": "Lädt",
    "button.submit": "OK",
    "button.upload.image": "Bild hochladen",
    "button.create": "Hinzufügen",
    "button.update": "Bearbeiten",
    "button.copy": "Kopieren",
    "button.delete": "Löschen",
    "button.save": "Speichern",
    "button.cancel": "Abbrechen",
    "button.close": "Schließen",
    "button.viewlist": "Vorschaubilder anzeigen",
    "button.map": "Auf Karte anzeigen",
    "button.tracking": "Verarbeitung läuft",
    "select.none": "keine Auswahl",
    "error.inUse": "Es wurde mehr als eine Zeile mit dem angegebenen Bezeichner gefunden",
    "error.sqlIntegrityConstaint": "Die angegebenen Daten haben nicht das richtige Format, um gespeichert zu werden. Bitte prüfen Sie die zugehörigen Einträge.",
    "error.unique": "Eintrag ist bereits mit einem anderen Objekt verknüpft. Bitte erstellen Sie einen neuen Eintrag oder löschen Sie die bestehende Beziehung.",
    "error.internalServerError": "Interner Server Fehler.",
    "error.accessdenied": "Zugriff verweigert.",
    "error.notexists": "Gewählter Eintrag existiert nicht.",
    "error.notfound": "Gewählter Eintrag existiert nicht.",
    "error.badrequest": "Prüfen, ob bei der Erstellung eines Objekts eine ungültige ID angegeben wurd.",
    "error.wrongInputValue": "Falscher Eingabe Wert.",
    "error.unauthorized": "Unbefugte Anfrage.",
    "error.invalidDefinition": "Ungültige Definition.",
    "error.general.delete": "Dieses Element konnte nicht gelöscht werden.",
    "error.general.create": "Dieses Element konnte nicht erstellt werden.",
    "error.general.update": "Dieses Element konnte nicht aktualisiert werden.",
    "error.general.get": "Dieses Element konnte nicht geladen werden.",
    "error.serverOffline": "Der Server scheint offline zu sein.",
    "error.userOffline": "Sie scheinen offline zu sein.",
    "error.unknown": "Unbekannter Fehler aufgetreten.",
    "error.apptemplate.notfound": "Die Vorlage konnte nicht gefunden werden.",
    "error.app.notfound": "Die Anwendung konnte nicht gefunden werden.",
    "error.entity.notfound": "Dieses Element konnte nicht gefunden werden.",
    "error.image.notunique": "Namen müssen gesetzt und eindeutig sein!",
    "error.image.renew.noredis": "Bild konnte nicht geladen werden. Es ist keine Valkey/Redis-Verbindung konfiguriert.",
    "error.image.renew.nostreamkey": "Bild konnte nicht geladen werden. Für diese Fläche ist kein SAE-Stream-Key gesetzt.",
    "error.image.renew.noframe": "Bild konnte nicht geladen werden. Im SAE-Stream ist kein aktuelles Bild verfügbar.",
    "error.image.renew.invalidframe": "Bild konnte nicht geladen werden. Das aktuelle Bild konnte nicht dekodiert werden.",
    "response.save.success": "Speichern erfolgreich.",
    "response.save.failed": "Speichern fehlgeschlagen!",
    "confirm.dialog.title": "Bestätigen des Dialog Titels",
    "confirm.dialog.description": "Bestätigen der Dialog Beschreibung",
    "map.live": "Livedaten anzeigen",
    "observationArea.showSavedTrajectories" : "Trajektorien anzeigen",
    "observationArea.recordTrajectories" : "Start / Stop Aufzeichnung",
    "observationArea.trajectoryFilter.noRecordings" : "Keine aufgezeichneten Trajektorien für diesen Stream verfügbar"
};
export default translationDeDE;
