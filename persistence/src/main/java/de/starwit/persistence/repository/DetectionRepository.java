...
public interface DetectionRepository extends CustomRepository<DetectionEntity, Long> {
    ...
    @Query("""
            SELECT new de.starwit.persistence.repository.TimestampBounds(
    ...
    @Query(value = """
            SELECT bucketIndex, count(*) AS objectCount
            FROM (
                SELECT DISTINCT
                    cast(floor(extract(epoch from (detection_timestamp - :start)) / :bucketSeconds) as bigint) AS bucketIndex,
                    object_id
                FROM detection
